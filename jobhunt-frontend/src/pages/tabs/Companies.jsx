...
function CompactCard({ company, onClick, selected }) {
  const matchColor =
  ...
  return (
    <motion.div
      className={`compact-card ${selected ? 'compact-card-selected' : ''}`}
...
export default function Companies() {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        setCompanies(mapped)
        setSelectedIndex(0) // reset to first on new data
  ...
  const featured = companies[selectedIndex] || companies[0] || null
  const sidebarCompanies = companies.filter((_, i) => i !== selectedIndex)
